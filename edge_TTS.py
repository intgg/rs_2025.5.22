...
async def play_audio_from_memory(audio_data):
    ...
    # 初始化pygame混音器 - 将移至主应用控制
    # mixer.init()

    if not mixer.get_init():
        print("错误: Pygame Mixer 未初始化。请在调用此函数前初始化 Mixer。")
        # 尝试进行一次默认初始化作为后备，但这应该由调用者处理
        print("尝试进行默认初始化...")
        try:
            mixer.init()
            if not mixer.get_init():
                print("默认初始化失败。音频无法播放。")
                return
            print("Mixer 已以后备方式初始化。")
        except Exception as e:
            print(f"后备初始化 Mixer 时出错: {e}")
            return
